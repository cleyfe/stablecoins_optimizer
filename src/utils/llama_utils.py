...
def plot_chain_differences(diff_df):
    ...
    fig.update_layout(xaxis_title='Timestamp', yaxis_title='Difference (Max APY Base - Min APY Base Borrow)', legend_title='Chain')
    return fig
...
def plot_overall_differences(merged_df):
    ...
    fig.update_layout(xaxis_title='Timestamp', yaxis_title='Values', legend_title='Metric')
    return fig
...
def print_strategy_stats(final_apy_df, number_of_loops):
    ...
    print(f"Number of tx per day: {cost_per_rebalancing*24}")
    cost_per_tx = 0.03
    print(f"cost per tx (conservative for a L2): ${cost_per_tx}")
    daily_gas_cost = cost_per_tx * cost_per_rebalancing * 24
